...
session_start();
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            ...
            if( !empty($name_update) &&
                ...
                if(strlen($_FILES['product-image']['name'])!=0){
                    ...
                    move_uploaded_file($_FILES['product-image']['tmp_name'],"product-image/".$_FILES['product-image']['name']);
                    unlink($product['image']);
                    $result = mysqli_query($conn,$toSql);
                    if(mysqli_query($conn,$toSql)){header("Location: store_seller.php"); die;}else{echo "Error";}
                }else{
    ...
    else{header("Location: store_seller.php");}
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <form method="post" enctype="multipart/form-data">
        ...
        <input type="text" name="product-name" value="<?php echo $product['product_name']?>">

        <label for="product-detail">Description</label>
        <input type="text" name="product-detail" value="<?php echo $product['product_detail']?>">

        <label for="product-price">Price</label>
        <input type="text" name="product-price" value="<?php echo $product['price']?>">

        <label for="product-amount">Amount</label>
        <input type="text" name="product-amount" value="<?php echo $product['amount']?>">

        <label for="product-image">Image</label>
        <label for="product-image">Current Image:<img src="<?php echo $product['image']?>"</label>
